//
//  main.swift
//  mlx-drunet-swift / DRUNetValidate
//
...
//
//  Usage:  swift run drunet-validate <weights.safetensors> [image.png] [width] [height]
//
...
guard let weightsPath = args.first else {
    print("usage: drunet-validate <weights.safetensors> [image.png] [width] [height]")
    exit(2)
}
...
func main() async {
    ...
    let config = DRUNetConfiguration(weightsURL: URL(fileURLWithPath: weightsPath))
    let request = ImageRestoreRequest(image: makeImage())

    do {
        let result = try await ValidationHarness.run(
            engine: engine,
            registration: DRUNetRestorePackage.registration,
            ...
            heartbeatLabel: "drunet")

        print("")
        print(result.run.splitLogLine("drunet-\(config.variant.rawValue)"))
        print("")
